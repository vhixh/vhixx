...
if selected =='Home':
    ...
    def home_page():
        ...
        st.markdown(
            ...
            <ul class="feature-steps">
                <li>Pilih <b> aplikasi</b> yang ingin diambil ulasannya.</li>
    ...
    if st.button("Mulai Scraping Ulasan"):
        if selected_apps:
            # Simpan ID aplikasi ke session state
    ...
    if "scraping_started" in st.session_state and st.session_state.scraping_started:
        ...
        # Progress bar dan status scraping
        ...
        st.info("Silakan pilih file untuk memulai scraping.")
    ...
    if uploaded_file is not None:
        ...
        # Pastikan kolom 'polarity' ada dalam dataset
        if 'polarity' in df.columns and 'text_steamengl' in df.columns:
            if st.button("Mulai Visualisasi"):
                ...
                def generate_wordcloud(data, title, color):
                    data = data.dropna().astype(str)  # Hapus nilai NaN dan pastikan semua data dalam bentuk string
                    text = ' '.join(data)
                    if text.strip():
    ...
    selected_file = st.selectbox("Pilih file untuk dianalisis dan visualisasi:", files_available)

    # Validasi jika pengguna belum memilih file
    if selected_file != " Pilih File ":
        # Membaca file yang dipilih
        file_path = os.path.join(folder_path, selected_file)
        st.write(f"File yang dipilih: {selected_file}")
        ...
        if st.session_state.analysis_started:
            ...
            if st.button("Mulai Analisis Ulang"):
                st.session_state.analysis_started = False  # Reset session state
